...
export default function Perfil() {
  ...
  const handleCancelEdit = () => {
    setIsEditing(false);
    // Restaurar datos originales desde localStorage
    const userStr = localStorage.getItem('user');
    if (userStr) {
      const user = JSON.parse(userStr);
      setUserData({
        nombre: user.name || localStorage.getItem('user_name') || "Usuario",
        email: user.email || localStorage.getItem('user_email') || "usuario@hefesto.com",
